...
- 指定フォルダが存在しない場合は自動的に作成されます
- 省略した場合は Vault のルートに保存されます
- `$キー名$` でフォーム入力値を使用できます

```
{{meta|folder=[$project_name$]}}
```
...
{{multiselect|tags|label=[タグ]|list=[重要;確認待ち;完了]|separator=[, ]}}
{{multiselect|tags|label=[タグ]|list=[重要;確認待ち;完了]|markdownlist=[-]}}
```

フォーム上にチェックボックス形式の複数選択 UI を表示します。
`list` オプションは必須です。省略すると致命的エラーになります。

**使用可能なオプション：** `label` `description` `default` `required` `list` `rows` `separator` `markdownlist`
...
`separator` と `markdownlist` は同時に指定できません。両方指定した場合は先に書かれた方が優先されます（警告が表示されます）。

**`separator` — 区切り文字で連結**

```
separator=[, ]   → 重要, 確認待ち, 完了
separator=[/]    → 重要/確認待ち/完了
separator=[ | ]  → 重要 | 確認待ち | 完了
```

省略時のデフォルトは `, `（カンマ＋半角スペース）です。

**`markdownlist` — Markdown リスト形式**
...
```
{{multiselect|tags|list=[Work;Personal;Study]|markdownlist=[-]}}
```

選択値が「Work」「Study」の場合の出力：
...
```
{{multilist|aliases}}
{{multilist|aliases|label=[エイリアス]|rows=[5]|markdownlist=[-]}}
{{multilist|notes|label=[メモ]|separator=[\n---\n]}}
```
...
**使用可能なオプション：** `label` `placeholder` `description` `default` `required` `rows` `separator` `markdownlist`

**出力形式の制御：**

`separator` と `markdownlist` は同時に指定できません（`multiselect` と同様）。
...
# 出力（markdownlist=[1.] 指定時）
1. 東京オフィス
2. Tokyo Office
3. 本社
```

**Frontmatter の aliases での使い方：**
...
| `separator=[区切り]` | 文字列 | multiselect / multilist | 出力時の区切り文字。`[]` 内の文字がそのまま使用される |
| `markdownlist=[-]` | `-` / `*` / `1.` | multiselect / multilist | Markdown リスト形式で出力 |
...
| 条件 | メッセージ例 |
|---|---|
| 未知のオプション名 | `Unknown option "requred" in field type "text". Did you mean "required"?` |
| 未定義の meta キー | `Unknown meta key: "tittle"` |
| `default` 値が `list` に存在しない | `Default value "未定" is not in the list of field "status"` |
| `separator` と `markdownlist` を同時指定 | `Both "separator" and "markdownlist" are set in field "tags". "separator" takes priority.` |
...
{{multiselect|skills|label=[スキル]|list=[剣術;魔法;弓術;回避;回復;索敵]|markdownlist=[-]}}
{{multilist|aliases|label=[別名・エイリアス]|markdownlist=[-]}}
...
{{multilist|attendees|label=[参加者]|separator=[, ]}}
...
**Q. Frontmatter の YAML が崩れる**

Form Builder はテンプレートエンジンとして文字列置換のみを行います。YAML の妥当性はチェックしません。`multiselect` や `multilist` で `markdownlist` を使用する場合、Frontmatter 内でのインデントを含めた配置はテンプレート側で設計してください。

**Q. `separator` と `markdownlist` を両方書いたらどうなる？**

先に記述されている方が優先され、後の方は無視されます。フォーム上部に警告メッセージが表示されます。

**Q. ファイル名に使えない文字を入力したらどうなる？**

OS の禁止文字（`/ \ : * ? " < > |`）は自動的に `_` に置き換えられ、通知メッセージが表示されます。
